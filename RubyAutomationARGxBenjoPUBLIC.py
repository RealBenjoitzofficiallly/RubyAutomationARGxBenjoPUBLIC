# Most of the code made by Java#9999 -  Modified by Benjo and ARG
import pip 
try:
    import discord
...
#global logs command (i made it because i was bored lol)
@bot.command()
@is_owner()
async def global_logs(ctx, status: str):
    with open('settings.json', 'r') as f:
        settings = json.load(f)

    if status.lower() == "on":
        settings['MISC']['GLOBAL_LOGS']['HIDDEN'] = True
        message = "Global Logs anonymity has been turned on."
    elif status.lower() == "off":
        settings['MISC']['GLOBAL_LOGS']['HIDDEN'] = False
        message = "Global Logs anonymity has been turned off."
    else:
    ...
    embed = discord.Embed(
        title="Global Logs anonymity Status Update",
    ...
    if await restart_main_py():
            print("Bot restarted after updating Global Logs anonymity")
    else:
            print("Error while trying to restart the bot after updating Global Logs anonymity")

            #max buy amount command
@bot.command(name='maxbuy')  
@is_owner()
async def maxbuy(ctx, new_maxbuy: str):
    try:
        new_maxbuy_float = float(new_maxbuy)
    except ValueError:
        embed = Embed(title=' ```The max buy amount must be a number. ```', color=Colour.from_rgb(255, 0, 0))
        await ctx.send(embed=embed)
        return
    ...
    if new_maxbuy_float.is_integer():
        new_maxbuy_str = str(int(new_maxbuy_float))
        new_maxbuy_value = int(new_maxbuy_float)
    else:
        new_maxbuy_str = str(new_maxbuy_float)
        new_maxbuy_value = new_maxbuy_float

   
    settings['MISC']['MAX_BUY_AMOUNT'] = new_maxbuy_value

    
    with open('settings.json', 'w') as f:
        json.dump(settings, f, indent=4)

    embed = Embed(title='Success!', description=f'```MAX_BUY_AMOUNT Value has been changed to: {new_maxbuy_str}```', color=Colour.from_rgb(255, 182, 193))
    await ctx.send(embed=embed)

    if await restart_main_py():
            print("Succesfully restarted mewt after updating the MAX_BUY_AMOUNT")
    else:
...
# arg and benjo snipe mode command
...
async def owners(ctx):
    ...
    # Add a field for the owners with mention and ID
    owners_info = [f"<@{owner_id}> ({owner_id})" for owner_id in authorized_ids]
    owners_str = "\n".join(owners_info)
    embed.add_field(name="Owners", value=owners_str, inline=False)

    # Send the embed as a message to the invoking user
    await ctx.send(embed=embed)
...
async def ssi(ctx):
    ...
    buy_debounce = settings["MISC"]["BUY_DEBOUNCE"]
    scan_speed = settings["MISC"]["WATCHER"]["SCAN_SPEED"]
    autosearch = settings["MISC"]["AUTOSEARCH"]["ENABLE"]
    
    
    embed = discord.Embed(title=f"Hi, {ctx.message.author.name}! 👋", color=discord.Color.from_rgb(65,105,225))
    embed.add_field(name="Buy Debounce:", value=buy_debounce, inline=False)
    embed.add_field(name="Scan Speed:", value=scan_speed, inline=False)
    embed.add_field(name="Autosearch:", value=autosearch, inline=False)
    embed.set_thumbnail(url="https://tenor.com/view/byuntear-sad-sad-cat-cat-meme-gif-25617057")
    embed.set_footer(text="MODIFIED BY: ARG. and real_benjosnipesthis")
...
async def autosearch(ctx, status: str):
    ...
    if await restart_main_py():
            ...
            #autosearch but less letters optional command
@bot.command()
@is_owner()
async def auto(ctx, status: str):
    with open('settings.json', 'r') as f:
        settings = json.load(f)
